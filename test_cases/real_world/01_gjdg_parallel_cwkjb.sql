...
SELECT 'A'                                                                                      ACTIONTYPE   , --操作类型
       ...
       CAST(SUBSTRING('{DATA_DT}',1,6) AS VARCHAR(6))                                          BUOCMONTH    , --报告期
       '0'                                                                                      D0101        , --业务类别
       country.d0102                                                                            D0102        , --对方国家/地区
       '2'                                                                                      D0103        , --对方部门
       '4'                                                                                      D0104        , --对方与本机构的关系
       '1'                                                                                      D0105        , --原始期限
       curr.cd_value1                                                                           D0106        , --原始币种
       NVL(ld01.d0109,0.00)                                                                     D0107        , --上月末本金金额
       NVL(ld01.d0111,0.00)                                                                     D0108        , --上月末应收利息金额
       dvlp.su_bal                                                                              D0109        , --本月末本金金额
       dvlp.su_bal                                                                              D0110        , --本月末本金金额：其中剩余期限在一年及以下
       dvlp.D0111                                                                               D0111        , --本月末应收利息余额
       dvlp.D0112                                                                               D0112        , --本月非交易变动
       (dvlp.su_bal+dvlp.D0111)-(NVL(ld01.d0109,0.00)+NVL(ld01.d0111,0.00))-dvlp.D0112          D0113        , --本月净发生额 =（本月末本金余额+本月末应收利息余额）-（上月末本金余额+上月末应收利息余额）-本月非交易变动
       dvlp.D0114                                                                               D0114        , --本月利息收入
       ''                                                                                       REMARK       , --备注
       'CWKJB'                                                                                  DATA_SOURCE  , --数据来源
       '11005293'                                                                               BANK_ID      , --机构代码号
       '20070709380'                                                                            OWNUSR       ,  --柜员号
       '郝坤'                                                                                    FALPERS      ,  --报送人
       '财务会计部'                                                                               FALDEPT         --报送部门
      ,2                                                                                        d0115--是否存在风险转移
      ,NULL                                                                                     d0116--风险转往国家/地区
      ,NULL                                                                                     d0117--风险转往部门
FROM (SELECT curr_type                                 curr_type   --数字形式码值
...
LEFT JOIN ra03.wgj_send_curr                curr        ON dvlp.curr_type=curr.std_cd_value
LEFT JOIN ra03.d01_cwkj_cc                  country     ON country.d0106=curr.cd_value1
...
SELECT 'A'                                                                                      ACTIONTYPE   , --操作类型
       ...
       'HKD'                                                                                    D0106        , --原始币种
       NVL(ld01.d0109,0.00)                                                                     D0107        , --上月末本金金额
       0.00                                                                                     D0108        , --上月末应收利息金额
       dvlp.wallet_aval_bal                                                                     D0109        , --本月末本金金额
       dvlp.wallet_aval_bal                                                                     D0110        , --本月末本金金额：其中剩余期限在一年及以下
       0.00                                                                                     D0111        , --本月末应收利息余额
       0.00                                                                                     D0112        , --本月非交易变动
       (dvlp.wallet_aval_bal+0.00)-(NVL(ld01.d0109,0.00)+0.00)-0.00                             D0113        , --本月净发生额 =（本月末本金余额+本月末应收利息余额）-（上月末本金余额+上月末应收利息余额）-本月非交易变动
       0.00                                                                                     D0114        , --本月利息收入
       ''                                                                                       REMARK       , --备注
       'SZRMBB'                                                                                 DATA_SOURCE  , --数据来源
       '11005293'                                                                               BANK_ID      , --机构代码号
       '20190510900'                                                                            OWNUSR       ,  --柜员号
       '于鑫芳'                                                                                  FALPERS      ,  --报送人
       '数字人民币部'                                                                             FALDEPT         --报送部门
      ,2                                                                                        d0115--是否存在风险转移
      ,NULL                                                                                     d0116--风险转往国家/地区
